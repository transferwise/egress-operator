...
func deployment(es *egressv1.ExternalService, configHash string) *appsv1.Deployment {
	...
	a["egress.monzo.com/admin-port"] = strconv.Itoa(int(adminPort(es)))

	return &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			Name:        es.Name,
			Namespace:   namespace,
			Labels:      deploymentLabels(es),
			Annotations: deploymentAnnotations(es),
		},
...
func podSpec(es *egressv1.ExternalService) corev1.PodSpec {
	if es.Spec.PodSpec != nil {
		return *es.Spec.PodSpec
	}
				...
				ReadinessProbe: &corev1.Probe{
					Handler: corev1.Handler{
						HTTPGet: &corev1.HTTPGetAction{
							Path:   "/ready",
							Port:   intstr.FromInt(int(adminPort(es))),
